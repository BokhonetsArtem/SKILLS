...
description: >-
  ...
  aif-security-checklist. Не для обычной настройки HTTP-клиента,
  ...
  frontend-a11y. Не для SSR/RSC/hydration и решения render-mode —
  это frontend-ssr. Не ставит пакеты и не открывает сырые
  secret-файлы.
---
...
- Нужен ли SSR / гидрация / render-mode, а не XSS / token / `cookies()` как auth → [`frontend-ssr`](../frontend-ssr/SKILL.md), этот скил дальше не веди.
