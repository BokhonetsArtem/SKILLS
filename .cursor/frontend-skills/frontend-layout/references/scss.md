...
2. **Глобальный SCSS** (без `.module.`) — только этот файл. Если рядом доминируют `*.module.css` (YEAHUB) — **не** применяй этот файл и **не** добавляй Sass.
...
| Не надо | Надо |
| --- | --- |
| Добавить Sass в YEAHUB / Tailwind-репо | стек, который уже есть |
